        ...
        with open(filename, 'r') as f:
            ...
            for i, layer_info in enumerate(layer_data[:-1]):
                ...
                if layer_info[0] == "Dense":
                    ...
                    L = layer.Conv2D(number=i,name="Conv"+str(i),n_kers=1,ker_sz=1,n_chans=1,wt_scale=1,activation=layer_info[1],reg=int(layer_info[2]),verbose=eval(layer_info[3]))
                    L.wts = np.asarray(eval(layer_info[4]))
                    L.b = np.asarray(eval(layer_info[5]))
                elif layer_info[0] == "MaxPooling2D":
                    L = layer.MaxPooling2D(number=i,name="Pool"+str(i),pool_size=int(layer_info[1]),strides=int(layer_info[2]),activation=layer_info[3],reg=int(layer_info[4]),verbose=eval(layer_info[5]))
...
class convNet4AccelBigKernels(Network):
    ...
    def __init__(self, input_shape=(3, 32, 32), n_kers=(32,), ker_sz=(9,), dense_interior_units=(100,),
        ...
        C = layer.Conv2DAccel(number=0,name="Conv",n_kers=n_kers[0],ker_sz=ker_sz[0],n_chans=n_chans,wt_scale=wt_scale,activation="relu",reg=reg,verbose=False)
